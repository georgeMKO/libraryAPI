...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
  ...
  private UsernamePasswordAuthenticationToken getAuthenticationToken(HttpServletRequest request) {
    ...
    String username = Jwts.parser().setSigningKey(SecurityConstants.SECRET.getBytes())
        .parseClaimsJws(token.replace(SecurityConstants.TOKEN_PREFIX, "")).getBody().getSubject();
    Optional<Customer> optionalCustomer = customerRepository.findByUsername(username);

    return optionalCustomer.map(customer -> new UsernamePasswordAuthenticationToken(customer, null,
